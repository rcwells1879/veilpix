...
test('uses official Seedance 2.5 beta rates for video-guided billing', () => {
    ...
    assert.equal(estimateSeedanceVeilPixCredits(pricingContext), 95.42);
});

test('every Seedance 2.5 pricing path preserves every package margin floor', () => {
    const cases = [
    ...
    for (const pricingCase of cases) {
        ...
        assertGenerationPricing(estimateSeedanceVeilPixCredits(context), kieCostUsd, pricingCase.resolution);
    }
});
        ...
        for (const resolution of Object.keys(SEEDANCE_PRICING[variant])) {
            for (const duration of [limits.min, limits.max]) {
                for (const referenceVideoDuration of [0, limits.max]) {
                    ...
                    assertGenerationPricing(estimateSeedanceVeilPixCredits(context), providerCostUsd, `${variant} ${resolution}`);
                }
            }
        }
    }
