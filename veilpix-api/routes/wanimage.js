...
router.post('/combine-photos', withImageUploadErrors(uploadMultiple), validateImageReferences('wanimage'), checkUserCredits, async (req, res) => {
    const startTime = Date.now();
    let usageLogged = false;
    ...
    try {
        ...
        const wanRequest = buildCombineRequest(uploadResult.urls, prompt, resolution, aspectRatio, nsfwFilter, x, y);
        const wanResponse = await callWanImageAPI(wanRequest);
        const normalizedResponse = normalizeResponse(wanResponse);
